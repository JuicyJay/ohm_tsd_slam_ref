		...
		_cellSize(grid->getCellSize())
{
	...
	_occGrid->info.origin.position.x 		= 0.0 - (static_cast<double>(_grid.getCellsX()) * static_cast<double>(_grid.getCellSize()) * 0.5 + xOffset); //bec of translation occgrid <-> tsdgrid
	...
	///@todo warum prvNh ohne string template <>
...
void ThreadGrid::eventLoop(void)
{
	...
	///@todo unsigned char* colorBuffer - std::vector?
